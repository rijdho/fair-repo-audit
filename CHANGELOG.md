...
- **Renamed FAIR Metadata Check** (was FAIR Readout), and the hosted version FAIR Metadata Check
  (Extended) (was FAIR Readout Server): the name now says what is scored, and the pair reads as one
  tool in two editions. The page title, the brand block, the three interfaces, the README,
  `CONTRIBUTING.md` and `CITATION.cff` carry the new names; the address `fair-repo-audit` and the
  DOI stay. The Zenodo record takes the name at the next release. Cache-bust `?v=51`.

- **"Part of Metaudits" moves up**, under rijdho.github.io in the brand block (house-style 1.0.4), out of the rail credits. Assets move to `?v=50`.
